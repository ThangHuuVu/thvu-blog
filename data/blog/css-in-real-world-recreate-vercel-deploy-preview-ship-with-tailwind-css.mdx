---
title: CSS in real world - Re-create Vercel Deploy. Preview. Ship. with Tailwind CSS
date: "2021-10-07"
...
images:
  [
    "/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/daniel-ging-28q5mxg4G7E-unsplash.jpg",
    "/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/foreground.png",
    "/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/background.png",
  ]
layout: PostLayout
---
...
<Image
  alt="Header"
  src="/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/daniel-ging-28q5mxg4G7E-unsplash.jpg"
  width={640}
  height={853}
/>
<p className="italic text-sm text-gray-600 dark:text-gray-400">
  Photo by Daniel Ging on <a href="https://unsplash.com/photos/28q5mxg4G7E">Unsplash</a>
</p>
...
Earlier this week I [tweeted](https://twitter.com/thanghvu/status/1443891436303183875) about a [Tailwind Play version](https://play.tailwindcss.com/DI05HDfEjt) of [Vercel](https://vercel.com/home) "Deploy. Preview. Ship" Hero Header. Here is how it looks like on Vercel Homepage:
...
<div className="flex items-center justify-center">
  <video
    ...
    controls
    loop
    src="/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/vercel_hero.mov"
  ></video>
</div>
...
Noted that each `span` element contains a descendant `span` and a pseudo-element. While the outer `span` contains information about gradient colors, the `::before` pseudo-element is holding the black text (background), and the inner `span` is holding the gradient text (foreground).

# Implementation

## Typography

- Font choice: Vercel uses Inter font.
...
We are making good progress! Now the text should have vivid gradient colors. [Here](https://play.tailwindcss.com/cglMPQeMQR) is how it should look like.
...
<div>
  <Image
    alt="Background"
    src="/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/background.png"
    width={640}
    height={343}
  />
  ...
  <Image
    alt="Background"
    src="/static/blog/css-in-real-world-recreate-vercel-deploy-preview-ship-with-tailwind-css/foreground.png"
    width={640}
    height={343}
  />
...
The figures show that the two animations overlapping each other. Before the foreground appears, the background smoothly fades in. This keeps running infinitely, with an 8-sec interval.

### `::before` pseudo element

Let's style the pseudo-element:
...
- Use `data content and `before:content-[attr(data-content)]`to fill content inside pseudo-elements. You should see the display is all black now, as the pseudo-element is overlaying the descendant`span`. This is expected.

### Animations

Animation utilities in Tailwind CSS are enabled by default. To write custom animation, go to `tailwind.config.js`. We need to:
...
It's time to apply the class back to our markup. [Here](https://play.tailwindcss.com/t8I7Cr4FCu) is how it should look like at the end:
...
<h1 class="py-14 text-[23vw] text-center sm:text-10xl leading-none select-none tracking-tightest font-extrabold">
  <span data-content="Develop." class="relative block before:content-[attr(data-content)] before:w-full before:z-0 before:block before:absolute before:top-0 before:px-2 before:bottom-0 before:left-0 before:text-center before:text-black before:animate-gradient-background-1">
    <span class="px-2 text-transparent bg-clip-text bg-gradient-to-br from-gradient-1-start to-gradient-1-end animate-gradient-foreground-1"> Develop.</span>
  </span>
  <span data-content="Preview." class="relative block before:content-[attr(data-content)] before:w-full before:z-0 before:block before:absolute before:top-0 before:px-2 before:bottom-0 before:left-0 before:text-center before:text-black before:animate-gradient-background-2">
    <span class="px-2 text-transparent bg-clip-text bg-gradient-to-br from-gradient-2-start to-gradient-2-end animate-gradient-foreground-2"> Preview.</span>
  </span>
  <span data-content="Ship." class="relative block before:content-[attr(data-content)] before:w-full before:z-0 before:block before:absolute before:top-0 before:px-2 before:bottom-0 before:left-0 before:text-center before:text-black before:animate-gradient-background-3">
    <span class="px-2 text-transparent bg-clip-text bg-gradient-to-br from-gradient-3-start to-gradient-3-end animate-gradient-foreground-3"> Ship.</span>
  </span>
</h1>
```
...
CSS, much like JS or any technologies in Web Dev, is an ever-learning subject; I can't get enough of it. You can see the source code of [this site Hero](https://github.com/ThangHuuVu/thvu-blog/blob/master/components/Hero.tsx) to see how I implemented it. Hope you enjoyed the read.
